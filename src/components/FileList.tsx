...
const FileList = ({ subjectId, subfolderId, title, backUrl, additionalActions, children }: FileListProps) => {
    ...
    const { data: files, isLoading } =
        trpc.getUserFiles.useQuery({ subjectId, subfolderId })
        ...
        <main className='mx-auto max-w-7xl md:p-10'>
            <div className='mt-8 flex flex-col items-start justify-between gap-4 border-b border-gray-200 pb-5 sm:flex-row sm:items-center sm:gap-0'>
                ...
                <div className="flex items-center gap-2">
                    {additionalActions}
                    <UploadButton subjectId={subjectId} subfolderId={subfolderId} />
                </div>
            </div>

            {/* Injected Content (e.g. Subfolders) */}
            {children}
